...
function SignUp() {
  ...
  return(
      <main className="flex md:flex-row w-screen font-rubik">
          <section className="hidden md:flex bg-heroSign bg-left-top bg-cover bg-no-repeat w-1/2 h-auto border overflow-x-hidden">
          </section>
          <section className=" w-full md:w-1/2 flex flex-col overflow-x-hidden">
            <div className="flex flex-col pt-10 ps-10 pe-12">
              ...
              <div className="flex flex-col m-20 mb-16 ">
                <form className="flex flex-col gap-y-3 " action="" onSubmit={(event) =>{submitHandler(event)}}>
                  <div className="flex flex-col gap-y-2">
                    <label className=" text-[#4F5665] font-bold text-[20px] " htmlFor="email" >Email Adress :</label>
                    <input className="text-[20px] border border-[#4F5665] rounded-[20px] py-5 px-5 focus:outline-none focus:border-brown focus:ring-brown focus:ring-1 placeholder:text-[#9F9F9F] placeholder:text-[20px] placeholder:font-normal" type="text" name="email" id="email" placeholder="Enter your email adress" required autoComplete="email" onChange={changeHanlder} /> 
                  </div>
                  <div className="flex flex-col gap-y-2">
                    <label className=" text-[#4F5665] font-bold text-[20px] " htmlFor="password">Password :</label>
                    <input className="text-[20px] border border-[#4F5665] rounded-[20px] py-5 px-5 focus:outline-none focus:border-brown focus:ring-brown focus:ring-1 placeholder:text-[#9F9F9F] placeholder:text-[20px] placeholder:font-normal" type="password" name="password" id="password" placeholder="Enter your password" required onChange={changeHanlder} />
                  </div>
                  <div className="flex flex-col gap-y-2">
                    <label className=" text-[#4F5665] font-bold text-[20px] " htmlFor="phone" >Phone Number :</label>
                    <input className="text-[20px] border border-[#4F5665] rounded-[20px] py-5 px-5 focus:outline-none focus:border-brown focus:ring-brown focus:ring-1 placeholder:text-[#9F9F9F] placeholder:text-[20px] placeholder:font-normal" type="tel" name="phone" id="phone" placeholder="Enter your phone number" required onChange={changeHanlder} />
                  </div>
                ...
                <a className=" text-center text-[20px] text-white font-bold bg-brown rounded-[20px] py-5 px-5 mt-12 shadow-shadow-button hover:bg-orange-950"  href="/login">Login Here</a>
              </div>
            </div>
            <FooterSign/>
          </section>
